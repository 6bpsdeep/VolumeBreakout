...
You now have **two Pine scripts**: `pine_script_50pct.pine` (50% buffer) and `pine_script_100pct.pine` (100% buffer). Both send to the **same** webhook URL — the dashboard tabs separate them by buffer tier automatically.

**One-time setup:**
1. In each Pine file, find the line `"secret":"REPLACE_WITH_YOUR_SECRET"` and replace with the `WEBHOOK_SECRET` value from Render.
2. Paste each into a separate Pine Editor window in TradingView, save them as two distinct indicators.

**For each Kuwait stock you want to monitor:**

1. Open the chart, **add the 50% indicator**, then right-click chart → **Add alert** → condition: 50% indicator → "Any alert() function call" → Webhook URL: `https://YOUR-RENDER-URL.onrender.com/webhook/breakout` → Save
2. **Add the 100% indicator** to the same chart, repeat the alert setup with the 100% indicator
3. You now have two alerts on this chart. The dashboard tabs will route them automatically.

Repeat for all watchlist stocks.
...
```
kw-breakout/
├── pine_script_50pct.pine   # TradingView indicator — 50% buffer (Standard)
├── pine_script_100pct.pine  # TradingView indicator — 100% buffer (Extreme)
...
└── client/
    ...
    └── src/
        ├── main.jsx
        ├── App.jsx          # Interactive table + buffer tabs + theme toggle
        └── index.css        # Dark + light theme dashboard styling
```
